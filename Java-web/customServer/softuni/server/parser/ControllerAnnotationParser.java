...
public class ControllerAnnotationParser implements AnnotationParser<HttpRequestMethod, Map<String, ControllerActionPair>> {
    ...
    private String createMappingRegex(Method currentMethod,
        ...
        for (int i = 0; i < mappingTokens.size(); i++) {
            if (!(mappingTokens.get(i).startsWith("{") &&
                    mappingTokens.get(i).endsWith("}"))) {
                continue;
            }
    ...
    public void parse(Map<HttpRequestMethod, Map<String, ControllerActionPair>> routes) throws
        ...
        Class[] classes = this.classProvider.getClassByAnnotation(Controller.class);
        for (Class currentClass : classes) {
            for (Method currentMethod : Arrays.stream(currentClass.getDeclaredMethods())
                    .filter(method -> method.isAnnotationPresent(RequestMapping.class))
                    .toArray(Method[]::new)) {

                RequestMapping requestMapping = currentMethod.getAnnotation(RequestMapping.class);
                HttpRequestMethod httpRequestMethod = requestMapping.method();
                ...
                ControllerActionPair actionPair = new ControllerActionPair(currentMethod, controllerInstance, argumentMapping);
                if (!routes.containsKey(httpRequestMethod)) {
                    routes.put(httpRequestMethod, new HashMap<>());
                }

                routes.get(httpRequestMethod).put(url, actionPair);
            }
        }
    }
}
